...
app = Flask(__name__, template_folder='testtemplates')
    ...
    if request.method == 'POST':
        ...
        for i in range(1, 5):
            if request.form.get(f'use_seq{i}') == 'on':
                ...
                if summ == 0:
                    continue  # Пропускаємо порожні послідовності

                # Розрахунки залежно від масштабу
                if scale == '10мм':
        ...
        #  підсумовування після обробки всіх колонок:
        for r in results:
            total['A'] += r['A']
            total['T'] += r['T']
            total['G'] += r['G']
        ...
        # ➤ Округлення фінальних результатів
        for k in total:
            total[k] = round(total[k], 2)

    return render_template('testindex.html', results=results, total=total)
